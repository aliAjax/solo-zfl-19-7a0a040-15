...
错误约定：校验失败 `400`，找不到资源 `404`，库存不足/超量/状态冲突 `409`，短缺量等细节放在 `details` 字段。请求体必须是 JSON 对象（`null`、数组、裸值一律 400）；长度类字段只接受数字（布尔、字符串、`null` 均拒绝）；批次号按归一化字符串判重，数字 `9001` 与 `"9001"` 视为同一批次不可重复入库。
